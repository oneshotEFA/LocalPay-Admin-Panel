...
export default function DepositsPage() {
  ...
  const { data, isLoading, error } = useDeposits(params);
...
console.log("Deposits data:", data);
  const getStatusBadge = (status: string) => {
    ...
    if (status === "VERIFIED")
      return (
        <Badge className="bg-teal-50 text-teal-700 border-teal-200 shadow-none gap-1 text-xs">
          <CheckCircle2 className="w-3 h-3" />
          Verified
        </Badge>
      );
    if (status.includes("REJECTED"))
      return (
        <Badge className="bg-rose-50 text-rose-700 border-rose-200 shadow-none gap-1 text-xs">
          <XCircle className="w-3 h-3" />
          Rejected
        </Badge>
      );
    return (
      <Badge className="bg-blue-50 text-blue-700 border-blue-200 shadow-none gap-1 text-xs">
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            Audit customer deposit attempts and verification results.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        <div className="relative max-w-sm">
          ...
          <Input
            placeholder="Search by ID or checkout ID…"
            className="pl-9 bg-white border-slate-200 h-9 text-sm shadow-sm"
            value={search}
            onChange={(e) => {
          ...
          <TableBody>
            {isLoading ? (
              ...
              items.map((dep) => (
                <TableRow
                  key={dep.id}
                  className="group border-slate-100 hover:bg-slate-50/60 cursor-pointer transition-colors relative"
                >
